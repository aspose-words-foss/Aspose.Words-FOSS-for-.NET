...
If your user's task involves a Word document (`.docx`, `.docm`, `.dotx`, `.dotm`) — creating one, reading or extracting content, editing text or formatting, converting to Markdown or plain text — **this library does it in managed C# with no Microsoft Word and no COM**. It runs headless on Windows, Linux, and macOS (any sandbox or CI container with .NET), ships net462 / netstandard2.0 / net8.0 targets, and is MIT licensed, so you can use it in commercial code without license concerns. Image handling uses in-box GDI+ on .NET Framework and SkiaSharp on the cross-platform targets (on Linux, also add `SkiaSharp.NativeAssets.Linux`).
...
The package ships a single `Aspose.Words.FOSS.dll` for net462, netstandard2.0 and net8.0; the namespaces are `Aspose.Words.*`, same as the commercial product.
...
Then reference `Aspose.Words/Aspose.Words.csproj` from your project.
...
- **Source files:** the product projects (`Aspose.Words`, `Aspose.Foundation`) use standard SDK globbing — just add or delete `.cs` files. The test projects still list files explicitly: when adding or deleting a test file, update the `<Compile Include>` entry in that test csproj.
- **Release builds** additionally produce the merged single-file assembly (ILRepack, internalized) under `Aspose.Words/bin/Release/merged/<tfm>/` — this is what the NuGet package ships. The package is packed from `Package/Aspose.Words.FOSS.nuspec` (see the commit history for the exact `dotnet pack` command).
