// Copyright (c) 2001-2026 Aspose Pty Ltd. All Rights Reserved.
// 02/11/2010 by Alexey Titov

using System;

namespace Aspose.Words.Drawing.Core.Dml.Guides
{
    internal class DmlTwoArgumentFormula : DmlFormula
    {
        internal DmlTwoArgumentFormula(string[] formulaParts, IDmlTwoArgumentFormulaCallback callback)
        {
            if (formulaParts.Length < 3)
                throw new ArgumentOutOfRangeException("formulaParts", "Wrong number of formula parts.");

            mX = formulaParts[1];
            mY = formulaParts[2];
            mCallback = callback;
        }

        internal DmlTwoArgumentFormula(string[] formulaParts, IDmlTwoArgumentFormulaCallback callback, string source)
            : this(formulaParts, callback)
        {
            Source = source;
        }

        /// <summary>
        /// Calculates the formula.
        /// </summary>
        public override double Calculate(IDmlGuideValueProvider guideValueProvider)
        {
            double x = DmlFormulaHelper.GetValue(X, guideValueProvider);
            double y = DmlFormulaHelper.GetValue(Y, guideValueProvider);
            return mCallback.Calculate(x, y);
        }

        internal string X
        {
            get { return mX; }
            set { mX = value; }
        }

        internal string Y
        {
            get { return mY; }
            set { mY = value; }
        }

        private readonly IDmlTwoArgumentFormulaCallback mCallback;
        private string mX;
        private string mY;
    }
}