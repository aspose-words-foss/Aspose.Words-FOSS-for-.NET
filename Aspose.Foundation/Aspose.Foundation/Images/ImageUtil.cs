...
namespace Aspose.Images
{
    ...
    public static class ImageUtil
    {
        ...
        internal static int DpiToPpm(double dpi)
        {
            return (int)(dpi * ImageConstants.InchesPerMeter);
        }

#if NETSTANDARD
        /// <summary>
        /// WORDSNET-16714 SkiaSharp does not write resolution into Jpeg files.
        /// Method writes resolution into JFIF segment, which seems always present in Jpeg produced by SkiaSharp.
        /// </summary>
        /// <param name="imageStream">Stream with image data in Jpeg format.</param>
        /// <param name="horizontalResolution">Horizontal resolution to be written.</param>
        /// <param name="verticalResolution">Vertical resolution to be written.</param>
        internal static void SaveJpegResolutionJfif(Stream imageStream, double horizontalResolution, double verticalResolution)
        {
            imageStream.Position = 0;
            ...
            try
            {
                ...
                BigEndianBinaryReader reader = new BigEndianBinaryReader(imageStream);
                JpegReader jpegReader = new JpegReader(reader);
                BigEndianBinaryWriter writer = new BigEndianBinaryWriter(imageStream);
                ...
                // Process segments of the JPEG file.
                jpegReader.GoToNextMarker();
                while (!resolutionFound)
                {
                    ushort segmentLength = reader.ReadUInt16();
                    long nextMarkerPosition = imageStream.Position + segmentLength - 2;

                    // APP0 Marker - possible JFIF segment.
                    if (jpegReader.CurrentMarker == 0xFFE0)
                    {
                        const int idLength = 5;
                        const int versionLength = 2;

                        imageStream.Seek(idLength + versionLength, SeekOrigin.Current);

                        // Write resolution unit and resolution values. 1 means dots per inch.
                        writer.WriteByte(1);
                        writer.WriteUInt16((ushort)horizontalResolution);
                        writer.WriteUInt16((ushort)verticalResolution);
            ...
            {
                // Seems to be broken Jpeg file or there is no JFIF segment. Do nothing with the image stream.
            }
        }
#endif
