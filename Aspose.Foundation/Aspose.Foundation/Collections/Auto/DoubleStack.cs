// Copyright (c) 2001-2026 Aspose Pty Ltd. All Rights Reserved.
// 20/08/2013 by Ivan Lyagin

// 
// This source code was auto-generated by Aspose.CsClassTransformer.ConsoleApp, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null.
// Do not change the file manually as your changes will be lost during next auto-generation session.
// 

using System;

namespace Aspose.Collections
{
    /// <summary>
    /// Represents a variable size last-in-first-out (LIFO) collection of instances of the same arbitrary type.
    /// </summary>
    /// <dev>
    /// This class was created using ROTOR sources. It was reworked considering AW specifics and Java autoportability.
    /// </dev>
    public class DoubleStack
    {
        /// <summary>
        /// Initializes a new instance of the DoubleStack class that is empty and has the default initial capacity.
        /// </summary>
        public DoubleStack()
        {
            mItems = gEmptyArray;
        }

        /// <summary>
        /// Initializes a new instance of the DoubleStack class that is empty and has the specified initial capacity 
        /// or the default initial capacity, whichever is greater.
        /// </summary>
        public DoubleStack(int capacity)
        {
            ArgumentUtil.CheckNonNegative(capacity, "capacity");
            mItems = new double[capacity];
        }

        /// <summary>
        /// Initializes a new instance of the DoubleStack class that contains elements copied from the specified array 
        /// and has sufficient capacity to accommodate the number of elements copied.
        /// </summary>
        public DoubleStack(double[] array)
        {
            ArgumentUtil.CheckNotNull(array, "array");
            mSize = array.Length;
            mItems = new double[mSize];
            array.CopyTo(mItems, 0);
        }

        /// <summary>
        /// Initializes a new instance of the DoubleStack class that contains elements copied from the specified DoubleStack 
        /// and has sufficient capacity to accommodate the number of elements copied.
        /// </summary>
        public DoubleStack(DoubleStack stack)
        {
            ArgumentUtil.CheckNotNull(stack, "stack");
            mSize = stack.mSize;
            mItems = new double[mSize];
            Array.Copy(stack.mItems, 0, mItems, 0, mSize);
        }

        /// <summary>
        /// Checks whether the stack is not empty. Throws if it is.
        /// </summary>
        private void CheckNotEmpty()
        {
            if (mSize == 0)
                throw new InvalidOperationException("The stack is empty.");
        }

        /// <summary>
        /// Removes all objects from the DoubleStack.
        /// </summary>
        public void Clear()
        {
            // Don't need to doc this but we clear the elements so that the gc can reclaim the references.
            Array.Clear(mItems, 0, mSize);
            mSize = 0;
        }

        /// <summary>
        /// Determines whether an element is in the DoubleStack.
        /// </summary>
        public bool Contains(double item)
        {
            if (mSize == 0)
                return false;

            return (Array.LastIndexOf(mItems, item, mSize - 1, mSize) >= 0);
        }

        /// <summary>
        /// Copies the DoubleStack to an existing one-dimensional array, starting at the specified array index.
        /// </summary>
        public void CopyTo(double[] array, int arrayIndex)
        {
            ArgumentUtil.CheckNotNull(array, "array");
            ArgumentUtil.CheckNonNegative(arrayIndex, "arrayIndex");
            if (arrayIndex + mSize > array.Length)
                throw new ArgumentException("The given array index specifies an invalid offset length.");

            for (int i = 0; i < mSize; i++)
                array[i + arrayIndex] = mItems[mSize - i - 1];
        }

        /// <summary>
        /// Returns the object at the top of the DoubleStack without removing it.
        /// </summary>
        public double Peek()
        {
            CheckNotEmpty();
            return mItems[mSize - 1];
        }

        /// <summary>
        /// Removes and returns the object at the top of the DoubleStack.
        /// </summary>
        public double Pop()
        {
            CheckNotEmpty();
            double item = mItems[--mSize];
            mItems[mSize] = gDefaultValue;
            return item;
        }

        /// <summary>
        /// Inserts an object at the top of the DoubleStack.
        /// </summary>
        public void Push(double item)
        {
            if (mSize == mItems.Length)
            {
                int newCapacity = (mItems.Length == 0) ? DefaultCapacity : (mItems.Length * 2);
                double[] newItems = new double[newCapacity];
                Array.Copy(mItems, 0, newItems, 0, mSize);
                mItems = newItems;
            }

            mItems[mSize++] = item;
        }

        /// <summary>
        /// Copies the DoubleStack to a new array.
        /// </summary>
        public double[] ToArray()
        {
            double[] array = new double[mSize];
            CopyTo(array, 0);
            return array;
        }

        /// <summary>
        /// Sets the capacity to the actual number of elements in the DoubleStack, if that number is less 
        /// than 90 percent of current capacity.
        /// </summary>
        public void TrimExcess()
        {
            int threshold = (int)(((double)mItems.Length) * 0.9);
            if (mSize < threshold)
            {
                double[] newItems = new double[mSize];
                Array.Copy(mItems, 0, newItems, 0, mSize);
                mItems = newItems;
            }
        }

        /// <summary>
        /// Determines whether the specified object is equal to the current object.
        /// </summary>
        public override bool Equals(object obj)
        {
            if (ReferenceEquals(null, obj))
                return false;
            if (ReferenceEquals(this, obj))
                return true;
            if (obj.GetType() != typeof(DoubleStack))
                return false;

            return Equals((DoubleStack)obj);
        }

        /// <summary>
        /// Determines whether the specified object is equal to the current object.
        /// </summary>
        private bool Equals(DoubleStack rhs)
        {
            if (ReferenceEquals(null, rhs))
                return false;
            if (ReferenceEquals(this, rhs))
                return true;

            if (Count != rhs.Count)
                return false;

            for (int i = 0; i < Count; i++)
            {
                if (mItems[i] != rhs.mItems[i])
                    return false;
            }

            return true;
        }
        
        /// <summary>
        /// Serves as the default hash function.
        /// </summary>
        /// <remarks>
        /// Note, this object is not immutable, so you should take your own care about it is not changed
        /// after this method is invoked, for example when it is used in a hashtable.
        /// </remarks>
        public override int GetHashCode()
        {
            int hashCode = Count;

            for (int i = 0; i < Count; i++)
                hashCode = unchecked(hashCode * 397 + mItems[i].GetHashCode());

            return hashCode;
        }


#if DEBUG
        /// <summary>
        /// Returns a string that represents the current object.
        /// </summary>
        //public override string ToString()
        //{
        //    return ArrayUtil.DumpArray(mItems, 0, Math.Min(20, Count));
        //}
#endif

        /// <summary>
        /// Gets the number of elements contained in the DoubleStack.
        /// </summary>
        public int Count
        {
            get { return mSize; }
        }

        private double[] mItems;
        private int mSize;

        private static readonly double[] gEmptyArray = new double[0];

        // The default item value. The field is not readonly to satisfy Java final.
        private static double gDefaultValue;

        private const int DefaultCapacity = 4;
    }
}
