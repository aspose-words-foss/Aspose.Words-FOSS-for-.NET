...
//
// last saved (in emacs): 
// Time-stamp: <2009-September-18 21:18:13>
//
// ------------------------------------------------------------------
//
...
namespace Aspose.Zip
{
    ...
    internal class ZlibBaseStream : Stream
    {
            ...
            else if ((_streamMode == StreamMode.Reader) && (_flavor == ZlibStreamFlavor.GZIP))
            {
                if (!_wantCompress)
                {
                    ...
                    if (_z.AvailableBytesIn != 8)
                        throw new ZlibException(String.Format("Protocol error. mAvailableBytesIn={0}, expected 8",
                                _z.AvailableBytesIn));
        ...
        public static byte[] CompressString(String s, Type type)
        {
            // workitem 8460
        ...
        public static String UncompressString(byte[] compressed, Type type)
        {
            // workitem 8460
        ...
        public static byte[] CompressBuffer(byte[] b, Type type)
        {
            // workitem 8460
            using (MemoryStream ms = new MemoryStream())
            {
        ...
        public static byte[] UncompressBuffer(byte[] compressed, Type type)
        {
            // workitem 8460
            byte[] working = new byte[1024];
            using (MemoryStream output = new MemoryStream())
