...
namespace Aspose.Charset
{
    ...
    /// </summary>
    public abstract class CharsetMultiByteRecognizer : CharsetRecognizer
    {
        /// <summary>
        ...
        /// <returns>Two values packed into one int  (Damn java, anyhow)</returns> 
        /// <br/>
        ...
        internal int Match(CharsetDetector det, int[] commonChars)
        {
            ...
            for (iter.Reset(); NextChar(iter, det);)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if ((commonChars != null) &&
                            (ArrayUtil.BinarySearch(commonChars, 0, commonChars.Length, (int)cv) >= 0))
                        {
                            commonCharCount++;
                        }
                    }
            ...
            if (doubleByteCharCount <= 10 && badCharCount == 0)
            {
                ...
                else
                {
                    ...
                    // WORDSNET-16775 Give commonCharCount a chance on short strings.
                    confidence = 10 + commonCharCount;
                }
        ...
        {
            internal int charValue = 0; // 1-4 bytes from the raw input data
        ...
        ///
        /// <param name="it">The IteratedChar "struct" into which the returned char is placed.</param>   
        /// <param name="det">The charset detector, which is needed to get at the input byte data being iterated over.</param>  
        /// <returns>True if a character was returned, false at end of input.</returns> 
        /// </summary>
        internal abstract bool NextChar(IteratedChar it, CharsetDetector det);

        private const double BadCharsThreshold = 0.03;
    }

    /// <summary>
    /// Shift-JIS charset recognizer.
    /// </summary>
    internal class CharsetRecog_sjis : CharsetMultiByteRecognizer
    {
        private static readonly int[] gCommonChars =
        ...
        internal override bool NextChar(IteratedChar it, CharsetDetector det)
        {
            ...
            // WORDSNET-23607 Check allowed characters in accordance with
            // https://en.wikipedia.org/wiki/Shift_JIS#With_vendor_or_JIS_X_0213_extensions
            // ASCII
            if (firstByte <= 0x80)
                return true;

            // Katakana
            if (firstByte >= 0xA0 && firstByte <= 0xDF)
                return true;

            // Not used as first byte, used by some single byte extensions.
            if (firstByte >= 0xFD && firstByte <= 0xFF)
                return true;

            // Double-byte
            if ((firstByte >= 0x81 && firstByte <= 0x9F) || (firstByte >= 0xE0 && firstByte <= 0xFC))
            {
        ...
        {
            int confidence = Match(det, gCommonChars);
    ...
    /// </summary>
    internal class CharsetRecog_big5 : CharsetMultiByteRecognizer
    {
        private static int[] commonChars =
            // TODO:  This set of data comes from the character frequency-
            //        of-occurence analysis tool.  The data needs to be moved
            //        into a resource and loaded from there.
        {
    ...
    /// </summary>
    internal abstract class CharsetRecog_euc : CharsetMultiByteRecognizer
    {
        /// <summary>
    ...
    /// </summary>
    internal class CharsetRecog_gb_18030 : CharsetMultiByteRecognizer
    {
        /// <summary>
        /// Get the next character value for EUC based encodings.
        /// Character "value" is simply the raw bytes that make up the character
